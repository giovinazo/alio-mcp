...
### 지금 바로 시작하기 — Claude Desktop 확장으로 설치 (권장)

1. **[Claude Desktop](https://claude.ai/download)** 설치 (이미 있으면 최신 버전으로 업데이트)
2. **[최신 릴리스](https://github.com/giovinazo/alio-mcp/releases/latest)** 에서 **`alio-mcp.mcpb`** 다운로드
3. Claude Desktop → **설정(Settings) → 확장(Extensions) → "고급 설정(Advanced settings)" → "Install Extension…"** → 받은 **`alio-mcp.mcpb` 선택** → **Install**. (`.mcpb`가 안 보이면 파일 형식을 "모든 파일"로) — **Windows에서 가장 확실한 방법입니다.**
4. 끝. 대화창에서 *"산단공 내부규정 찾아줘"* 처럼 물어보세요.

> ⚠️ **Windows 사용자** — `.mcpb` **더블클릭은 "연결 프로그램 없음"**, **설정 창에 드래그앤드롭도 무반응**일 수 있습니다(파일 형식 미등록 + 빈 확장 목록엔 드롭 대상이 없어 조용히 실패). 위 3번 **"Install Extension…" 파일 선택** 방식을 쓰세요. (macOS는 더블클릭·드래그앤드롭도 대체로 동작합니다.)
...
**Python·Node 두 런타임으로 동일한 17개 도구를 제공한다.** Node판(`node/`)은 Python을 1:1 포팅(도구별 동등성 적대 검증 완료)해 **MCPB 번들(`.mcpb`)** 로 패키징 — Claude Desktop 확장 설치용(설정 → 확장 → 고급 설정 → Install Extension)이며, esbuild 단일 번들이라 Node 외 의존성이 없다.
...
- **v1.0.0** (2026-05-30) — **Node/TypeScript 포팅(`node/`) + MCPB(`.mcpb`) 배포.** Claude Desktop 확장으로 클릭 몇 번에 설치(Python·터미널·설정 파일 편집 불필요). 도구 11개 Python↔TS 동등성 적대 검증 통과(critical 0). 번들 엔트리는 `.mjs`로 두어 Node 18+ 모든 환경에서 ESM 실행 보장. 기존 Python 코어(`alio_core.py`)·alio-crawler 공유 구조는 그대로 유지.
